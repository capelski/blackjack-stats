...
When(
  'getting the bet multiplier for a blackjack hand after splitting',
  function (this: BetMultiplierWorld) {
    this.result = getBetMultiplier({ isBlackjack: true, isDoubleBet: true });
  },
);

When(
  'getting the bet multiplier for a hand that doubled the bet',
  function (this: BetMultiplierWorld) {
...
When('getting the bet multiplier for a hand that surrendered', function (this: BetMultiplierWorld) {
  this.result = getBetMultiplier({ isSurrender: true });
});
